...
where table_schema = database() and
      column_name = 'Correlation_IdTwo' and
      table_name = @tableName;

set @query = IF(
    @exist <= 0,
    concat('alter table ', @tableName, ' add column Correlation_IdTwo varchar(38) character set ascii'), 'select \'Column Exists\' status');
...
set @query = IF(
    @column_type_IdTwo <> 'varchar(38) character set ascii',
    'call sqlpersistence_raiseerror(concat(\'Incorrect data type for Correlation_IdTwo. Expected varchar(38) character set ascii got \', @column_type_IdTwo, \'.\'));',
...
where
    table_schema = database() and
    index_name = 'Index_Correlation_IdTwo' and
    table_name = @tableName;

set @query = IF(
    @exist <= 0,
    concat('create unique index Index_Correlation_IdTwo on ', @tableName, '(Correlation_IdTwo)'), 'select \'Index Exists\' status');
